...
if ($options['ie9fix']) {
    theme_foundation_recurse_svgs($path, '', 'theme_foundation_svgtool_ie9fix', $blacklist);

} else if ($options['noaspectratio']) {
    theme_foundation_recurse_svgs($path, '', 'theme_foundation_svgtool_noaspectratio', $blacklist);
...
function theme_foundation_svgtool_ie9fix($file) {
    global $CFG;
...
function theme_foundation_svgtool_noaspectratio($file) {
    global $CFG;
...
function theme_foundation_recurse_svgs($foundation, $sub, $filecallback, $blacklist) {
    if (is_dir("$foundation/$sub")) {
        $items = new DirectoryIterator("$foundation/$sub");
        foreach ($items as $item) {
            if ($item->isDot()) {
                continue;
            }
            $file = $item->getFilename();
            theme_foundation_recurse_svgs("$foundation/$sub", $file, $filecallback, $blacklist);
        }
        unset($item);
        unset($items);
        return;

    } else if (is_file("$foundation/$sub")) {
        if (substr($sub, -4) !== '.svg') {
            return;
        }
        $file = realpath("$foundation/$sub");
